...
describe('customFilter', () => {
  ...
  it('removes also `null` values', () => {
    expect(
      customFilter(['foo', undefined, 'bar', null], true, true)
    ).toEqual(['foo', 'bar'])
  })

  it("removes also empty values", () => {
    expect(
      customFilter(['foo', undefined, 'bar', null, ''], true, true, true)
    ).toEqual(['foo', 'bar'])
  })
  ...
  it("removes also `'0'` values", () => {
    expect(
      customFilter(['foo', undefined, 'bar', null, false, '', 0, '0'], true, true, true, true, true, true)
    ).toEqual(['foo', 'bar'])
  })
})
